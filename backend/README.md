...
两块能力：
1. **列线图自动拟合**：从原始病例数据拟合 **Logistic / Cox** 回归，生成前端列线图所需的「变量分值 + 总分→概率」结构（rms 风格）。前端「从数据拟合」调用。
2. **账号 + 云同步**：注册/登录、按用户备份/恢复项目。前端「云同步」调用。

**数据库两栖**：本地（桌面内置）用 SQLite；设了环境变量 `DATABASE_URL` 时自动切 Postgres。
**免费上云**：账号同步部分可免费部署到 Render + Neon，见 **[DEPLOY.md](DEPLOY.md)**（云端用精简依赖 `requirements-cloud.txt`，不含统计库）。
