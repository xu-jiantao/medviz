...
export default function AggregatePage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      for (const u of users) {
        const ws = await idbGet<{ patient?: { name: string; bed: string; diagnosis: string } }>(
  ...
  const onExport = () => {
    ...
    downloadWorkbook(`全部用户病人数据汇总.xlsx`, [{ name: '汇总', aoa }])
    message.success(`已导出 ${rows.length} 个用户的数据`)
  }
