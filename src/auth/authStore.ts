...
export const SEED_ACCOUNTS: { username: string; password: string; email: string; role: Role; label: string }[] = [
  { username: 'superadmin', password: 'super8831', email: 'super@medviz.app', role: 'superadmin', label: '超级管理员' },
  { username: 'admin', password: 'admin8832', email: 'admin@medviz.app', role: 'admin', label: '管理员' },
  { username: 'doctor', password: 'doctor8833', email: 'doctor@medviz.app', role: 'doctor', label: '医生' },
  { username: 'demo', password: 'demo8834', email: 'demo@medviz.app', role: 'user', label: '普通用户' },
]
...
async function ensureSeedAccounts() {
  for (const a of SEED_ACCOUNTS) {
    ...
    const existing = await idbGet<StoredUser>(k)
    if (existing) {
      // 检查当前存储的密码哈希是否与代码中的一致，如果不一致则自动更新，以确保修改内置密码后本地数据库同步生效
      const ok = await verifyPassword(a.password, existing.salt, existing.hash)
      if (!ok) {
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  register: async ({ username, email, password, question, answer }) => {
    ...
    await checkAndStampLogin(username, 'user')
